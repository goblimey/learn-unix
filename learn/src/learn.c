 ...
 *
 * Updated and tested under Linux by Simon Ritchie
 * (www.merrowinternet.com) March 2002.  My changes are tagged
 * with the comment "SAR".
 */
...
#include "config.h"	/* SAR March 2002 */
#include <fcntl.h>
#include <stdio.h>
#include <signal.h>
#include <string.h>
...
/* #include "pathnames.h" (Defined data directory.  Now configured externally */

char	*direct	= PKGDATADIR;
int	more;
char	*level;
int	speed;
char	*sname;
...
/*
 * Emulate the system function mkdtemp() in systems that don't offer it.
 * Some UNIX systems (OpenBSD?), offer mkdtemp(), which chooses a unique name
 * and uses it to create a temporary directory.  Other systems have only
 * mktemp(), which creates a unique name.  Use that to choose the name,
 * and create the directory.
 *
 * Mktemp() derives the unique name from the process-id, but some old
 * versions have quite a high chance of using the same name.
 * This creates potential races when two processes use the same name at
 * once.  Modern system such as Linux and Solaris choose from a much
 * more complex set of potential names, making the method fairly safe.
 *
 * SAR March 2002.
 */
...
{
	char * dir;	/* save template from being overwritten by mktemp() */

	dir = strdup(template);

	dir = mktemp(dir);	/* choose name */
	...
	if (dir != NULL) {
		...
		if (mkdir(dir, 0744) != 0) {	/* create directory */
...
FILE *fin;
{
	...
	if (subdir[0]==0)
		snprintf(subdir, sizeof subdir, "%s/%s", PKGDATADIR, sname);
	for (;;) {
...
retry:
	start(todo);
	/* script = lesson */
	snprintf(tbuff, sizeof tbuff, "%s/%s/L%s", PKGDATADIR, sname, todo);
	scrin = fopen(tbuff, "r");
	if (scrin == NULL) {
...
maktee()
{
	...
	if (tee[0] == 0)
		snprintf(tee, sizeof tee, "%s/tee.learn", LIBEXECDIR);
	pipe(fpip);
	in = fpip[0];
	out= fpip[1];
	if (fork() == 0) {
...
char *argv[];
{
	...
	if ((playdir=mkdtemp(strdup("/tmp/learn.playpen.XXXXXX"))) == NULL ||
	     chdir(playdir) < 0) {
...
selunit()
{
	...
	while (ask) {
		...
		if (strcmp(dobuff, "bye") == 0)
			wrapup(0);
		level = todo = dobuff;
		snprintf(s, sizeof s, "%s/%s/L%s", PKGDATADIR, sname, dobuff);
		if (access(s, R_OK) == 0)
			return;
...
retry:
	f=scrin;
	if (f==NULL) {
		snprintf(fnam, sizeof fnam, "%s/%s/L%s", PKGDATADIR, sname, level);
		f = fopen(fnam, "r");
		if (f==NULL) {
...
start(char *lesson)
{
	...
	if (ask)
		return;
#endif
	snprintf(where, sizeof where, "%s/%s/L%s", PKGDATADIR, sname, lesson);
